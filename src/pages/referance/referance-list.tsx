...
import type { ReferanceResponse } from "@/types/referance.types";

// Sortable Row Component
interface SortableRowProps {
  item: ReferanceResponse;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm md:text-base text-muted-foreground dark:text-foreground/70 mt-1">
            Referansları görüntüleyin ve yönetin
          </p>
        </div>
        <Button onClick={() => navigate("/referance/create")} className="w-full sm:w-auto">
                    ...
                    >
                      {items.map((item) => (
                        <SortableRow
                          key={item.id}
                          item={item}
                          onView={(id) => navigate(`/referance/${id}`)}
                          onEdit={(id) => navigate(`/referance/${id}/edit`)}
